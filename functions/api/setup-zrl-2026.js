...
export async function onRequestGet({ env }) {
    ...
    try {
        ...
        // 2. Inserimento Dati Iniziali (Seed)
        // Creiamo la stagione se non esiste
        await env.DB.prepare(`
            INSERT INTO zrl_seasons (id, name, is_active) 
            SELECT 1, 'ZRL 2025/26', 1 
            WHERE NOT EXISTS (SELECT 1 FROM zrl_seasons WHERE id = 1)
        `).run();

        // Creiamo il Round Group 1 (ID 19) se non esiste
        await env.DB.prepare(`
            INSERT INTO zrl_round_groups (series_id, round_index, external_season_id, description) 
            SELECT 1, 1, 19, 'ZRL Round 1 (Spring)' 
            WHERE NOT EXISTS (SELECT 1 FROM zrl_round_groups WHERE external_season_id = 19)
        `).run();

        return new Response(JSON.stringify({ 
            success: true, 
            message: "Setup e Inizializzazione dati (Season 19) completati con successo." 
