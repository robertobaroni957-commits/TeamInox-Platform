...
from utils.auth_decorators import require_role, require_admin, require_captain
...
def manage_captains():
    ...
    if request.method == "POST":
        ...
        zwift_power_id = request.form.get("zwift_power_id", type=int)

        if action == "assign_captain" and team_id and zwift_power_id:
            # Assegna il capitano al team
            cur.execute("""
                UPDATE teams
            ...
            # Imposta is_captain = 1 sul rider
            cur.execute("""
                UPDATE riders
                SET is_captain = 1
            ...
            # Aggiorna is_captain solo se non è capitano in altri team
            cur.execute("""
                UPDATE riders
                SET is_captain = CASE
                    WHEN EXISTS (
                        SELECT 1 FROM teams WHERE captain_zwift_id = ?
                    ) THEN 1
                    ELSE 0
                END
                WHERE zwift_power_id = ?
    ...
    # GET: selezione team per filtro (facoltativo)
    selected_team_id = request.args.get("team_id", type=int)

    # Rider disponibili: solo chi non è capitano
    ...
    # Capitani attuali (riders che sono capitani in almeno un team), unici
    current_captains = cur.execute("""
        SELECT DISTINCT r.zwift_power_id, r.name
        FROM riders r
        JOIN teams t ON r.zwift_power_id = t.captain_zwift_id
        ORDER BY r.name
    """).fetchall()
